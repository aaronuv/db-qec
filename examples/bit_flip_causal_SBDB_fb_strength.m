...
tic
% current state:
% implementation of SBDB causal fb
% d rho = -i[dH^E, rho] + [[dH^EC_DB, rho - P_Q], rho]
%       - (gamma^EC/2)[Omega2,[Omega2,rho]] dt
%       - gamma^EC [Omega2, [H^(2),EC, rho]] dt
% the EC jumps are Luispe's

% WARNING: varying fb strength only works for Hermitian C^\EC
...
function dP_causal = build_causal_drift(P, Omega2, A, B, gamma, dt)
    % H^{(2),EC} = i B + i [A, rho]
    H2 = 1i * B + 1i * bracket(A, P);
...
function dP = build_update_P( ...
    ...
    Omega2_grid, gamma_grid, gamma_fb, dt)
    ...
    dP_causal = build_causal_drift(P, Omega2_grid{4}, A_grid{4}, B_grid{4}, gamma_grid{4}, dt) ...
        + build_causal_drift(P, Omega2_grid{5}, A_grid{5}, B_grid{5}, gamma_grid{5}, dt) ...
        + build_causal_drift(P, Omega2_grid{6}, A_grid{6}, B_grid{6}, gamma_grid{6}, dt);

    P_pivot = P + dbracket_total;
    [v, ~] = eigs(P_pivot, 1, 'largestreal');
    P_pivot = v*v';

    dPQ_pivot = P_pivot - gamma_fb * Q;
    ...
    dP_causal_pivot = build_causal_drift(P_pivot, Omega2_grid{4}, A_grid{4}, B_grid{4}, gamma_grid{4}, dt) ...
        + build_causal_drift(P_pivot, Omega2_grid{5}, A_grid{5}, B_grid{5}, gamma_grid{5}, dt) ...
        + build_causal_drift(P_pivot, Omega2_grid{6}, A_grid{6}, B_grid{6}, gamma_grid{6}, dt);
...
Omega2_grid{4} = gamma_fb * (-1i * B_grid{4} - 1i * bracket(A_grid{4}, Q));
Omega2_grid{5} = gamma_fb * (-1i * B_grid{5} - 1i * bracket(A_grid{5}, Q));
Omega2_grid{6} = gamma_fb * (-1i * B_grid{6} - 1i * bracket(A_grid{6}, Q));

% H^{(2),EC}_t = i B + i [A, rho]  (built inside build_causal_drift)
