...
function dP = build_update_P( ...
    ...
    % --- dH^EC : channels 4,5 (SB + DB) in -i[dH^EC, rho] ---
    ...
    dOmega = Omega2_grid{1} * dy{4} + Omega2_grid{2} * dy{5};
...
Omega2_grid{1} = gamma_fb * (1i * pi / 4) * (S1 * T1);
Omega2_grid{2} = gamma_fb * (1i * pi / 4) * (S2 * T2);
