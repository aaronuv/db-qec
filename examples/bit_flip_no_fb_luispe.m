...
% current state:
% 3-qubit bit-flip code for jumps C=Luispe
% and no feedback
...
function dP = build_update_P( ...
    ...
    % --- dH^EC : channels 4,5,6 (SB + DB) in -i[dH^EC, rho]; DB uses P (no -Q) ---
    ...
    dHtotal = dHSB_error + dHDB_error + dHSB_EC + dHDB_EC;
    ...
    dHtotal_pivot = dHSB_error + dHDB_error + dHSB_EC + dHDB_EC;
